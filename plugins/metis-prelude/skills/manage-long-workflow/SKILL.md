...
- **Local compute longer than about five minutes:** invoke
  `$metis-prelude:run-long-job` and
  end the agent turn after its bounded launch check.
- **Multi-agent decomposition:** invoke `$metis-prelude:orca-fleet`.
- **Fresh-context transfer:** invoke `$metis-prelude:handoff` at a major
  milestone.
...
a milestone. Prefer `$metis-prelude:handoff` and a fresh closeout task when the
transcript is large or the next phase needs little prior reasoning.
